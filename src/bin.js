...
function help() {
    console.log(`Usage: mockapy [options]

    Options:
            -p,  --port <port>        Port (default: 3000)
            -d, --database <file>    Database file (default: db.json)
            -r, --rule <file>        Rule file (default: rules.py)
...
function args() {
    try {
        const options = {
            ...
            rule: { type: 'string', short: 'r', default: process.env['RULE'] ?? path.join(__dirname, '..', 'rules.py') },
            help: { type: 'boolean' }
        };
...
(async () => {
    const rule = await Rule.createFromFile(rulePath);
